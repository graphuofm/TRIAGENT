...
**Where the difference shows up empirically.** A single-model confidence score fails in a specific, measurable way: the model can be confidently wrong. TriAgent's four-quadrant analysis quantifies it. In the *ambiguous* quadrant — where FinBERT's and Qwen-7B's scores differ by more than 0.7, 15.9% of Financial PhraseBank — Qwen-7B is right only **28%** of the time, below chance for a three-way task, while stating a mean confidence of 0.93. A confidence threshold on the LLM would not flag these rows. FinBERT is right on 71% of them. **When the specialist and the LLM diverge sharply, trust the specialist.** That case is invisible to any signal derived from one model.
